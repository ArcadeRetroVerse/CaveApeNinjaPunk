...
var hearts = []; // Array to store heart objects
var xchcoins = 0;
var cats = 0; // Track the number of cats
var lives = 3;
var velocityY = 0;
var gravity = 0.2;
var jumpPower = 10; // Power for each jump
var apeY = canvas.height - 150; // Start ape near the bottom
var gameOver = false;
var lastObstacleTime = 0; // To track when the last obstacle was created
...
var obstacles = [];
var obstacleWidth = 59;
var obstacleHeight = 59;

var obstacleInterval = 6000; // Start with 6 seconds interval for obstacles
var lastObstacleSpawnTime = Date.now(); // Keep track of when obstacles spawn

var countdownInterval = undefined; // Declare the interval variable globally to clear it when needed
var isWebviewOpen = false; // Flag to check if webview is open
var isInInstructions = false; // Instructions
...
function submitScore() {
	...
	leaderboard.push({ nickname: nickname, score: xchcoins });
...
var ape = {
	...
	handleCollision: function handleCollision() {
		var _this = this;

		// If collision detected, make the body red for 2 seconds
		if (this.isColliding) {
			setTimeout(function () {
				_this.isColliding = false; // Reset collision state after 2 seconds
...
var apiUrl = 'https://api.v2.tibetswap.io/token/76f8a24339230dcf8f83d79a6d7823929671f9dcc564583ae17f5aafaeceb68c';
	...
	var ballChain = {
		...
		draw: function draw() {
			...
			for (var i = 0; i < this.numBalls; i++) {
				var ballAngle = this.angle + i * (Math.PI / this.numBalls); // Spacing balls apart

				var ballX = prevX + Math.cos(ballAngle) * this.ballRadius;
				var ballY = prevY + Math.sin(ballAngle) * this.ballRadius;
		...
		checkCollision: function checkCollision() {
			...
			for (var i = 0; i < this.numBalls; i++) {
				...
				if (ape.x < ballX + 10 && ape.x + ape.width > ballX - 10 && ape.y < ballY + 10 && ape.y + ape.height > ballY - 10) {
...
function createHeart() {
	var heart = {
		...
		checkCollision: function checkCollision() {
			if (ape.x < this.x + this.width && ape.x + ape.width > this.x && ape.y < this.y + this.height && ape.y + ape.height > this.y) {
				return true; // Collision detected
			}
			return false; // No collision
		}
...
function createObstacle() {
	var obstacleType = Math.random() < 0.5 ? 'bombsec' : 'nftball'; // Randomly choose obstacle type
	var obstacle = {
		x: canvas.width,
...
function update() {
	...
	coins.forEach(function (coin, index) {
		...
		coin.draw();
		if (ape.x < coin.x + coin.width && ape.x + ape.width > coin.x && ape.y < coin.y + coin.height && ape.y + ape.height > coin.y) {
	...
	if (Date.now() - lastObstacleSpawnTime > 30000) {
		// After 30 seconds, reduce the interval
...
function startCountdownAndClose() {
	...
	var countdown = 9; // Set the countdown to 9 seconds
	var timerElement = document.getElementById('timer'); // Get the timer display element
...
function createOffer() {
	...
	fetch(apiUrl, {
		...
		setTimeout(function () {
			return closeWebview();
		}, 9000); // Simulate waiting for a response
...
function fetchTokenDetails() {
	fetch(apiUrl).then(function (response) {
		...
		if (data && data.asset_id && data.name) {
			...
			var tokenImageContainer = document.getElementById('tokenImage');
